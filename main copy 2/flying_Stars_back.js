class FylingStar_back {
   ...
   bigRipUpdate() {
      // Berechne die Richtung von der Mitte (3D-Vektor) zur aktuellen Position
      let center = createVector(0, 0, 0); // Mitte in 3D (bei z = 0)
      let direction = p5.Vector.sub(this.pos, center); // Richtung von der Mitte
      direction.normalize(); // Richtung auf eine Länge von 1 setzen
   
      // Geschwindigkeit proportional zur Entfernung
   
      // Bewege den Stern in Richtung der berechneten Richtung
      this.pos.x += direction.x ;
      this.pos.y += direction.y ;

      if (
         abs(this.pos.x) > width + 50 ||
         abs(this.pos.y) > height + 52)
      {
         this.toDraw = false;
       }
   }
